...
async def async_setup_entry(
    ...
    if device.device_type == DeviceType.AROMA_LINK:
        ...
        if device.supports_radar:
            entities.append(DiffuserRadarModeSensor(device, entry))
            entities.append(DiffuserRadarLevelSensor(device, entry))
...
class DiffuserRadarModeSensor(SensorEntity):
    """Radar or app work mode (Aroma-Link 52 0A)."""

    _attr_has_entity_name = True
    _attr_name = "Radar mode"
...
class DiffuserRadarLevelSensor(SensorEntity):
    ...
    _attr_icon = "mdi:radar"
    _attr_device_class = SensorDeviceClass.ENUM
    _attr_options = list(RADAR_LEVELS.values())
    ...
    @property
    def native_value(self) -> str | None:
        return RADAR_LEVELS.get(self._device.state.radar_level)
